...
func (r *MoviesRepository) FindByIdAdmin(c context.Context, id int) (models.Movie, error) {
	...
	for rows.Next() {
		...
		if views != nil {
			...
			movie.ViewsYouTube = new(int64) // Устанавливаем 0, если NULL
		}
...
func (r *MoviesRepository) Create(c context.Context, movie models.Movie) (int, error) {
	...
	row := tx.QueryRow(c,
		` 
    insert into movies(title, description, release_year, director, trailer_url, poster_url, duration, video_url, screen_src)
