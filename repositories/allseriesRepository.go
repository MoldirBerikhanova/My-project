...
func (r *AllSeriesRepository) Create(c context.Context, serie models.AllSeries) (int, error) {
	...
	row := r.db.QueryRow(c, "insert into allseries (series, title, trailer_url, duration) values($1, $2, $3, $4, $5, $6, $7, $8) returning id", serie.Series)
	err := row.Scan(&id)
	if err != nil {
		l := logger.GetLogger()
		l.Error(err.Error())
...
func (r *AllSeriesRepository) FindById(c context.Context, id int) (models.AllSeries, error) {
	var allserie models.AllSeries
	row := r.db.QueryRow(c, "select id, series,  title, trailer_url, duration from allseries where id = $1", id)
